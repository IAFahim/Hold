...
namespace Moves.Move.Debug
{
    ...
    public partial struct EaseVisualizerJob : IJobEntity
    {
        ...
        )
        {
            ...
            if ((leading3Bit & 0b001) != 0) VisualizePositions(current, ref blob, nextIndex, currentConfig, ref easeCacheArray);
        }

        private void VisualizePositions(
            byte current,
            ref EaseCacheBlob blob,
            byte nextIndex,
            EaseCache currentConfig,
            ref BlobArray<EaseCache> easeCacheArray
        )
        {
            int limit = 0;
            var end = current;
            while (limit != LoopLimit)
            {
                ...
                if ((leading3Bit & 0b001) == 0) break;
                currentConfig = easeCacheArray[current];
                nextIndex = currentConfig.Next;
                limit++;
            }
        }
            ...
            in EaseCache currentConfig)
        {
            ...
            using (Drawing.WithLineWidth(2f))
            {
                ...
                for (int i = 1; i <= Segment - 1; i++)
                {
                    ...
                    currentConfig.Ease.Evaluate(progress, out var t);
                    var rot = math.nlerp(startRot, endRot, t);
