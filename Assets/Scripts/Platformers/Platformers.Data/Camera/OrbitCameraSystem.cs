...
public partial struct OrbitCameraSystem : ISystem
{
    public struct CameraObstructionHitsCollector : ICollector<ColliderCastHit>
    {
        ...
        public CameraObstructionHitsCollector(Entity followedCharacter,
            DynamicBuffer<OrbitCameraIgnoredEntityBufferElement> ignoredEntitiesBuffer, float3 cameraDirection)
        {
            NumHits = 0;
            ClosestHit = default;
    ...
    public partial struct OrbitCameraJob : IJobEntity
    {
            ...
            in DynamicBuffer<OrbitCameraIgnoredEntityBufferElement> ignoredEntitiesBuffer)
        {
            ...
            if (LocalToWorldLookup.TryGetComponent(cameraControl.FollowedCharacterEntity, out var characterLTW) &&
                CustomGravityLookup.TryGetComponent(cameraControl.FollowedCharacterEntity,
                    out var characterCustomGravity) &&
                PlatformerCharacterComponentLookup.TryGetComponent(cameraControl.FollowedCharacterEntity,
                    out var characterComponent) &&
                PlatformerCharacterStateMachineLookup.TryGetComponent(cameraControl.FollowedCharacterEntity,
                    out var characterStateMachine))
            {
                // Camera target handling
                {
                    ...
                    if (LocalToWorldLookup.TryGetComponent(selectedCameraTarget, out var camTargetLTW))
                        selectedCameraTargetTransform =
                    ...
                    if (calculateUpFromGravity)
                        selectedCameraTargetTransform.rot = MathUtilities.CreateRotationWithUpPriority(
                    ...
                    // Update transitions
                    if (elapsedTime < orbitCamera.CameraTargetTransitionStartTime +
                        orbitCamera.CameraTargetTransitionTime)
                    {
                        float3 previousCameraTargetPosition = default;
                        if (LocalToWorldLookup.TryGetComponent(orbitCamera.PreviousCameraTarget,
                                out var previousCamTargetLTW))
                            previousCameraTargetPosition = previousCamTargetLTW.Position;
                        else
                            previousCameraTargetPosition = characterLTW.Position;

                        var transitionRatio =
                            math.saturate((elapsedTime - orbitCamera.CameraTargetTransitionStartTime) /
                                          orbitCamera.CameraTargetTransitionTime);
                        orbitCamera.CameraTargetTransform.pos = math.lerp(previousCameraTargetPosition,
                            selectedCameraTargetTransform.pos, transitionRatio);
                        orbitCamera.CameraTargetTransform.rot = math.slerp(
                            orbitCamera.CameraTargetTransitionFromTransform.rot, selectedCameraTargetTransform.rot,
                            transitionRatio);
                    }
                    else
                    {
                ...
                // Rotation
                {
                            ...
                            out var characterBody))
                    {
                        KinematicCharacterUtilities.AddVariableRateRotationFromFixedRateRotation(
                            ref localTransform.Rotation, characterBody.RotationFromParent, TimeData.DeltaTime,
                            characterBody.LastPhysicsUpdateDeltaTime);
                        orbitCamera.PlanarForward = math.normalizesafe(
                            MathUtilities.ProjectOnPlane(MathUtilities.GetForwardFromRotation(localTransform.Rotation),
                                cameraTargetUp));
                    }
                ...
                orbitCamera.SmoothedTargetDistance = math.lerp(orbitCamera.SmoothedTargetDistance,
                    ...
                    MathUtilities.GetSharpnessInterpolant(orbitCamera.DistanceMovementSharpness, TimeData.DeltaTime));
                ...
                localTransform.Position = orbitCamera.CameraTargetTransform.pos +
                                          -cameraForward * orbitCamera.ObstructedDistance;
